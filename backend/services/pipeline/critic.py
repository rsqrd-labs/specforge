...
def _demo_day_per_stage_focus(stage_type: str) -> str:
    ...
    if stage_type == "spec":
        return (
            ...
            "answer, a missing/boilerplate concept-differentiator clause in "
            "Overview, or a specific fact/API/capability claim that is not "
            "grounded in the problem statement or dependable general "
            "knowledge, is a ShallowSection defect; no implementation leakage "
    ...
    if stage_type == "plan":
        return (
            ...
            "flag ADRIncomplete only against this shorter shape (an ADR naming "
            "a tool/library capability that isn't real is a ShallowSection "
            "defect too); Environment "
            ...
            "or a REAL call with no fallback is a ShallowSection defect; "
            "Scalability and Performance addresses responsiveness "
            "(streaming/caching/model-size) when the product has an "
            "interactive user-facing path, not only scale/throughput — flag "
            "ShallowSection if it ignores this; Security Architecture names "
            "the concrete vulnerability class each validation point defends "
            "against (e.g. SQL injection, XSS) rather than a bare 'input is "
            "validated' claim — flag ShallowSection if it doesn't; no "
