...
This is the *user's* repo harness, distinct from SpecForge's own contract tests.

Stack handling is the load-bearing subtlety. The canonical SpecForge workspace is
...
CI_WORKFLOW_PATH = ".github/workflows/specforge.yml"
_STUB_DIR = "tests/specforge"
...
def build_pr_body(
    *,
    ...
    lines.append("")
    lines.append("*Generated by [SpecForge](https://specforge.ai)*")
    return "\n".join(lines) + "\n"
...
_CI_WORKFLOW_HEADER = """\
# Generated by SpecForge — the executable harness for this workspace.
name: SpecForge Harness
...
_PYTHON_JOB = """\
  python-harness:
    ...
    steps:
      ...
      - name: Run the SpecForge harness (red until implemented)
        run: pytest -q
"""
...
_PYTHON_STUB = """\
# SpecForge task stub — task_ref: {task_ref}
# {ref}: {title}
#
